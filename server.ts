...
// In-memory patient screening cases store (persisted during server session)
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'ok',
    service: 'MATLAB Diabetic Retinopathy Screening Pipeline',
    toolboxes: [
...
app.get('/api/cases/validations', (req, res) => {
...
app.post('/api/ai/clinical-opinion', async (req, res) => {
  try {
    ...
    const ai = getGenAI();
    if (!ai) {
      // High-quality deterministic clinical synthesis fallback
      return res.json({
...
app.post('/api/simulink/simulate', (req, res) => {
  try {
    ...
    // Image Transmission Time (2 eyes * 2.1 MB = 4.2 MB)
    const imageSizeBits = 4.2 * 8 * 1024 * 1024;
    const uploadTimeSec = imageSizeBits / (bandwidthMbps * 1024 * 1024);

    // Doctor capacity: 6 hours pure review time per day
    ...
    // Screening simulation over 52 weeks
    let currentQueue = 0;
    ...
    for (let week = 1; week <= 52; week++) {
      // Slight seasonal variation in rural camp turnout
      ...
      const referable = Math.round(arrivals * 0.18); // 18% prevalence in Indian diabetics
      totalReferable += referable;

      // Cases requiring central ophthalmologist review
      let casesToReview = 0;
      if (useEdgeAiTriage) {
        // Edge AI filters 80% non-referable cases locally!
        // Central queue receives referable (18%) + borderline quality checks (4%)
        casesToReview = Math.round(arrivals * 0.22);
      } else {
        // Without Edge AI, 100% of cases sent to central reading pool
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`MATLAB DR Screening Server running at http://0.0.0.0:${PORT}`);
  });
}
